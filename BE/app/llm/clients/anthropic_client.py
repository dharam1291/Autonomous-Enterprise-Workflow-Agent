...
class AnthropicClient:
    ...
    def extract_entities(self, text: str, config: ProviderConfig) -> ClaimEntities:
        ...
        raw = self._json_message(entity_extraction_prompt(text, config), config)
        try:
            payload = EntityExtractionPayload.model_validate(raw)
        except ValidationError as exc:
            logger.error(
                "Anthropic entity extraction returned JSON that does not match "
            ...
            raise LLMProviderError(
                f"Anthropic entity extraction payload failed schema validation "
